...
def is_valid_radio_port(comport):
    '''
    check if the given port is a valid EdgeTx/OpenTx radio port
    '''
    if not comport:
        return False
    try:
...
def open_passthrough(comport = None, baudrate = 115200, wirelessbridge = None):
    ...
    # always validate/auto-detect the radio port - ignore invalid ports passed in
    if comport and is_valid_radio_port(comport):
        radioport = comport
        print('Your radio is on com port', radioport)
    else:
